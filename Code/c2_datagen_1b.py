...
c2_datagen_1b.py
================
Generate training data for Model 1b (MC expected IRS value).

For each (FEDS date, a, sigma, t_k):
    1. Simulate N_OUTER outer paths from t=0 to t=10, record r at each T_MONITOR.
    2. At monitoring date t_k, batch all N_OUTER starting conditions with M_INNER
       replicates and run inner MC.
    3. label = mean over outer paths of mean(V_MC per inner batch)
             = E[V_MC(t_k)]
...
Output
------
  data/train_1b.h5  keys: X_disc, X_scalar, y_price
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Model 1b data generation — MC E[V] labels")
    p.add_argument("--feds",    type=str, default=FEDS_FILE)
    p.add_argument("--out",     type=str,
                   default=os.path.join(CODE_DIR, "data", "train_1b.h5"))
